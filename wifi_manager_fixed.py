...
"""
            ...
            # Use safer networking restart approach
            ...
            # Restart dhcpcd (non-blocking)
            try:
                subprocess.run(['systemctl', 'restart', 'dhcpcd'], check=True, timeout=10)
            except subprocess.TimeoutExpired:
                self.logger.warning("dhcpcd restart timed out, continuing...")
            except subprocess.CalledProcessError:
                self.logger.warning("dhcpcd restart failed, continuing...")
            
            # Start wpa_supplicant
            try:
                subprocess.run(['systemctl', 'start', 'wpa_supplicant'], check=True, timeout=10)
            except subprocess.TimeoutExpired:
                self.logger.warning("wpa_supplicant start timed out, continuing...")
            except subprocess.CalledProcessError:
                self.logger.warning("wpa_supplicant start failed, continuing...")
            ...
            if 'ESSID:' in result.stdout and 'Not-Associated' not in result.stdout:
                ...
                self.logger.warning(f"Failed to connect to {network['ssid']}")
                return False
    ...
    def setup_access_point(self, ap_config):
        ...
        try:
            self.logger.info("Setting up Access Point mode...")
...
"""
    ...
    def run(self):
        ...
        if not known_networks:
            self.logger.warning("No known networks configured, switching to AP mode")
